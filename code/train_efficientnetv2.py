...
################### 模型訓練 ###################
def Inception_create_and_fit_model(train_generator, validation_generator, summary=True, fit=True, epochs=EPOCHS):
    model = Sequential()
    conv_base = EfficientNetV2L(weights='imagenet', include_top=False, input_shape=(300, 300, 3))
    model.add(conv_base)
    model.add(Flatten())
    ...
    callbacks = [EarlyStopping(monitor="val_loss", patience=10, verbose=1, mode="min"), 
             ModelCheckpoint(filepath="mymodel_EfficientNet.h5", monitor="val_loss", mode="min", save_best_only=True, save_weights_only=False, verbose=1)]
...
# 建立並訓練模型
model, history = Inception_create_and_fit_model(train_generator, validation_generator)

# 載入模型
model = tf.keras.models.load_model('mymodel_EfficientNet.h5')
