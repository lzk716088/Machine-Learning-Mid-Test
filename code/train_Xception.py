...
################### 模型訓練 ###################
def Xception_create_and_fit_model(train_generator, validation_generator, summary=True, fit=True, epochs=EPOCHS):
    model = Sequential()
    ...
    callbacks = [EarlyStopping(monitor="val_loss", patience=10, verbose=1, mode="min"), 
                ModelCheckpoint(filepath="mymodel_Xception.h5", monitor="val_loss", mode="min", save_best_only=True, save_weights_only=False, verbose=1)]
...
model, history = Xception_create_and_fit_model(train_generator, validation_generator)

# 載入模型
model = tf.keras.models.load_model('mymodel_Xception.h5')
